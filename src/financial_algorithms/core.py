...
def slsqp_portfolio(mu, cov, risk_aversion=3.0, x0=None):
    n = len(mu)
    x0 = np.ones(n) / n if x0 is None else np.asarray(x0, float)
    res = minimize(
        lambda w: -utility(w, mu, cov, risk_aversion),
        x0,
        method="SLSQP",
        bounds=[(0, 1)] * n,
...
def sa_portfolio(mu, cov, risk_aversion=3.0, steps=5000, seed=7):
    ...
    n = len(mu)
    w = np.ones(n) / n
    score = utility(w, mu, cov, risk_aversion)
    best = w.copy()
    bests = score
    for k in range(steps):
        t = max(1e-5, 0.05 * (1 - k / steps))
        z = _project_simplex(w + rng.normal(0, 0.08, n))
        s = utility(z, mu, cov, risk_aversion)
        if s > score or rng.random() < np.exp((s - score) / t):
            w, score = z, s
        if score > bests:
            best, bests = w.copy(), score
    return best


def hybrid_portfolio(mu, cov, risk_aversion=3.0, steps=5000, seed=7):
    return slsqp_portfolio(
        mu, cov, risk_aversion, sa_portfolio(mu, cov, risk_aversion, steps, seed)
    )


def nonconvex_utility(
    w,
    mu,
    cov,
    previous=None,
    ...
    w = np.asarray(w, float)
    previous = np.ones(len(w)) / len(w) if previous is None else np.asarray(previous, float)
    turnover = np.abs(w - previous).sum()
    active = w > 1e-6
    small = np.logical_and(active, w < min_position).sum()
    excess = max(int(active.sum()) - max_assets, 0)
    return (
        utility(w, mu, cov, risk_aversion)
        - transaction_cost * turnover
        - cardinality_penalty * (small + excess)
    )
...
def sa_nonconvex_portfolio(mu, cov, previous=None, steps=12000, seed=7, **kwargs):
    ...
    n = len(mu)
    w = np.ones(n) / n
    score = nonconvex_utility(w, mu, cov, previous, **kwargs)
    best = w.copy()
    bests = score
    for k in range(steps):
        temp = max(2e-5, 0.035 * (1 - k / steps) ** 2)
        z = _project_simplex(w + rng.normal(0, 0.07, n))
        # occasional sparse/cardinality jump makes the search genuinely non-smooth
        if rng.random() < 0.18:
            keep = rng.choice(n, size=rng.integers(2, min(5, n + 1)), replace=False)
...
def hybrid_nonconvex_portfolio(mu, cov, previous=None, steps=12000, seed=7, **kwargs):
    ...
    w = sa_nonconvex_portfolio(mu, cov, previous, steps, seed, **kwargs)
    support = w > 1e-6
    idx = np.flatnonzero(support)
    sub_mu = np.asarray(mu)[idx]
    sub_cov = np.asarray(cov)[np.ix_(idx, idx)]
    refined = slsqp_portfolio(
        sub_mu, sub_cov, kwargs.get("risk_aversion", 3.0), w[idx] / w[idx].sum()
    )
    out = np.zeros(len(mu))
    out[idx] = refined
    ...
    return (
        out
        if nonconvex_utility(out, mu, cov, previous, **kwargs)
        >= nonconvex_utility(w, mu, cov, previous, **kwargs)
        else w
    )
...
def monte_carlo_dcf(
    base_cf, discount_rate=0.10, growth_mu=0.04, growth_sigma=0.08, n_sims=10000, seed=7
):
    rng = np.random.default_rng(seed)
    base = np.asarray(base_cf, float)
    vals = []
    for _ in range(n_sims):
        shocks = rng.normal(growth_mu, growth_sigma, len(base))
        cf = np.empty_like(base)
        cf[0] = base[0] * (1 + shocks[0])
        for i in range(1, len(base)):
            cf[i] = cf[i - 1] * (1 + shocks[i])
...
def trend_features(prices, window=40):
    ...
    short = np.mean(r[-5:])
    medium = np.mean(r[-20:]) if len(r) >= 20 else np.mean(r)
...
def fit_trend_model(price_series, window=40, horizon=5, ridge=1.0):
    p = np.asarray(price_series, float)
    X = []
    yy = []
    for t in range(window, len(p) - horizon):
        X.append(trend_features(p[: t + 1], window))
        yy.append(float(p[t + horizon] > p[t]))
    X = np.asarray(X)
    yy = np.asarray(yy)
...
def forecast_probability(prices, window=20, horizon=5, model=None):
    ...
    if model is not None:
        f = trend_features(p, model["window"])
        score = float(np.r_[1.0, f] @ model["coef"])
        return float(1 / (1 + np.exp(-2 * score)))
    ret = np.diff(np.log(p))
    sigma = float(np.std(ret[-max(window, 2) :], ddof=1))
    y = np.log(p[-window:])
    slope = np.polyfit(np.arange(window), y, 1)[0]
    z = slope * np.sqrt(horizon) / (sigma + 1e-12)
