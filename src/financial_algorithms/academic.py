...
from .core import dcf_value, portfolio_stats
...
def risk_metrics(w, mu, cov, alpha=0.95, rf=0.0):
    ...
    return {
        ...
        "sharpe": (r - rf) / (v + 1e-15),
        "var95": float(-(r - z * v)),
        "cvar95": float(-(r - v * norm.pdf(z) / (1 - alpha))),
    }
...
def efficient_frontier(mu, cov, n_points=30):
    ...
    for target in np.linspace(mu.min(), mu.max(), n_points):
        ...
        if res.success:
            ...
            out.append((v, r, res.x.copy()))
    return out


def sensitivity_grid(cashflows, discount_rates, terminal_growth_rates):
    return np.array(
        [[dcf_value(cashflows, r, g) for g in terminal_growth_rates] for r in discount_rates]
    )
...
def trend_features(prices, window=40):
    ...
    y = np.log(p[-window:])
    ret = np.diff(y)
    vol = np.std(ret, ddof=1) + 1e-12
    return np.array(
        [
            np.polyfit(np.arange(window), y, 1)[0] / vol,
            np.mean(ret[-5:]) / vol,
            np.mean(ret[-20:]) / vol,
...
def fit_probabilistic_trend(prices, window=40, horizon=1, ridge=0.5):
    p = np.asarray(prices, float)
    X = []
    y = []
    for t in range(window, len(p) - horizon):
        X.append(trend_features(p[: t + 1], window))
        y.append(float(p[t + horizon] > p[t]))
    X = np.asarray(X)
    y = np.asarray(y)
    mean = X.mean(0)
    scale = X.std(0) + 1e-12
    ...
    def objective(b):
        score = A @ b
        return float(
            np.mean(np.logaddexp(0, score) - y * score) + 0.5 * ridge * np.sum(b[1:] ** 2) / len(y)
        )
...
def bootstrap_ci(values, n_boot=2000, seed=17):
    ...
    samples = np.array([np.mean(rng.choice(x, len(x), replace=True)) for _ in range(n_boot)])
